...
const JWKS = createRemoteJWKSet(new URL(`${process.env.NEXT_PUBLIC_CLIENT_API}/api/auth/jwks`));
...
const run = async () => {
    try {
        ...
        app.post('/api/subscription', async (req, res) => {
            ...
            await userCollections.updateOne({ _id: new ObjectId(userId) }, { $set: { plan: 'pro' } });
        ...
        app.patch("/api/users/block/:id", async (req, res) => {
            ...
            const result = await userCollections.updateOne({ _id: new ObjectId(id) }, {
                $set: {
